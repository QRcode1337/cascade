      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Pre-built templates from the CASCADE playbook library - {templates.length} templates
          </p>
        </div>
      ...
      {needsSeeding ? (
        <div className="rounded-lg border bg-card p-12 text-center">
          ...
          <p className="text-muted-foreground text-sm mb-4">
            Click "Seed Agents" on the Agents page to load templates from the playbook library
          </p>
                ...
                {categoryTemplates.map((template) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className={`text-xs px-2 py-1 rounded-full ${categoryColors[template.category] || 'bg-muted text-muted-foreground'}`}>
                        {template.category}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {template.agents.slice(0, 3).map((agent) => (
                        <span key={agent} className="text-xs px-2 py-0.5 rounded bg-muted">
                          {agent}
                        </span>
                      ))}
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          {Object.entries(categoryColors).map(([cat, color]) => (
            <div key={cat} className="flex items-center gap-2">
              <span className={`w-3 h-3 rounded-full ${color.replace('text-', 'bg-').split(' ')[0]}`} />
