...
interface RunDetailPageProps {
  params: Promise<{ workspaceId: string; playbookId: string; runId: string }>;
}
...
export default async function RunDetailPage({ params }: RunDetailPageProps) {
  const { workspaceId, playbookId, runId } = await params;
  ...
  return (
    <div className="mt-6">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <span
          className={`px-4 py-2 rounded-lg border ${getStatusColor(run.status)}`}
        >
          {run.status}
        </span>
      </div>
          ...
          <div className="rounded-lg border bg-card p-6">
            ...
            <dl className="space-y-4 text-sm">
              ...
              <div>
                ...
                <dd className="font-medium mt-1">
                  {run.startedAt ? getDuration(run.startedAt, run.finishedAt) : '—'}
                </dd>
              </div>
            </dl>
          </div>
